...
namespace Nop.Admin.Controllers
{
    public partial class CountryController : BaseAdminController
    {
        ...
        {
            var cities = _stateProvinceService.GetStateProvincesByCountryId(countryid,true);
            //var gridModel = new DataSourceResult
            //{
            ...
            states.Add(new SelectListItem { Text = _localizationService.GetResource("Admin.Address.SelectState"), Value = "0" });
        ...
        public ActionResult EditCity(int id)
        {
            ...
            var country = _stateProvinceService.GetStateProvinceById(model.StateID).CountryId;
            ...
            AddLocales(_languageService, model.Locales, (locale, languageId) =>
            {
                locale.Name = city.GetLocalized(x => x.CityName, languageId, false, false);
            });
            var zipcodes = _zipcodeService.GetZipcodesByCityId(model.CityID).Select(x => x.ZipcodeNumber).ToList();
            string zip = "";
            if (zipcodes != null)
            {
            ...
            model.zipcodesbulk = zip;
            //Stores
            //PrepareStoresMappingModel(model, city, false);
            return View(model);
        }
        [HttpPost, ParameterBasedOnFormName("save-continue", "continueEditing")]
