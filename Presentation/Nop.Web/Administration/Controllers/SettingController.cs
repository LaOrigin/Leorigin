...
namespace Nop.Admin.Controllers
{
    public partial class SettingController : BaseAdminController
	{
        ...
        //Imp: Please revise [ValidateInput(false)]
        [HttpPost]
        [ValidateInput(false)]
        public ActionResult Catalog(CatalogSettingsModel model)
        {
            ...
            if (storeScope == 0)
                _settingService.SaveSetting(catalogSettings, x => x.BookedAmount, storeScope, false);
            else if (storeScope > 0)
                _settingService.DeleteSetting(catalogSettings, x => x.BookedAmount, storeScope);

            if (storeScope == 0)
                _settingService.SaveSetting(catalogSettings, x => x.TermsAndConditions, storeScope, false);
            else if (storeScope > 0)
                _settingService.DeleteSetting(catalogSettings, x => x.TermsAndConditions, storeScope);

            if (storeScope == 0)
                _settingService.SaveSetting(catalogSettings, x => x.OrderSteps, storeScope, false);
            else if (storeScope > 0)
                _settingService.DeleteSetting(catalogSettings, x => x.OrderSteps, storeScope);

            if (storeScope == 0)
                _settingService.SaveSetting(catalogSettings, x => x.PercentConfirmAmount, storeScope, false);
            else if (storeScope > 0)
                _settingService.DeleteSetting(catalogSettings, x => x.PercentConfirmAmount, storeScope);
