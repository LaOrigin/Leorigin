    ...
    public partial class ProductController : BaseAdminController
    {
            ...
            bool setPredefinedValues, bool excludeProperties)
        {
            ...
            if (product != null)
            {
                ...
                model.TermsAndConditions = product.TermsAndConditions;
            }
        ...
        [HttpPost]
        public ActionResult CityEditPopup(string btnId, string formId, ProductModel.CityModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();

            var sp = _productCitiesService.GetProductsCitiesById(model.Id);
            if (sp == null)
                //No state found with the specified id
                throw new ArgumentException("No product found with the specified id");
            ...
            if (model.StateId != 0 && model.CountryId != 0 && model.CityId != 0)
            {
                sp.CityID = model.CityId;
                sp.ProductID = model.ProductId;
                _productCitiesService.UpdateProductCities(sp);
        ...
        [HttpPost]
        public ActionResult AllCityList(int stateid)
        {
            var cities = _cityService.GetCityByStateProvincesId(stateid, true);
            //var gridModel = new DataSourceResult
            //{
            ...
            city.Add(new SelectListItem { Text = _localizationService.GetResource("Admin.Address.SelectCity"), Value = "0" });
            foreach (var c in cities)
                city.Add(new SelectListItem { Text = c.CityName, Value = c.CityID.ToString() });
            return Json(city);
        }
        [HttpPost]
        ...
        [HttpPost]
        public ActionResult CityUpdate(ProductModel.CityModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();

            var productcity = _productCitiesService.GetProductsCitiesById(model.Id);
            if (productcity == null)
                throw new ArgumentException("No tier price found with the specified id");

            var product = _productService.GetProductById(productcity.ProductID);
            if (product == null)
                throw new ArgumentException("No product found with the specified id");
            ...
            productcity.CityID = model.CityId;
            productcity.ProductID = model.ProductId;
            _productCitiesService.UpdateProductCities(productcity);
        ...
        public ActionResult CityDelete(int id)
        {
            ...
            var city = _productCitiesService.GetProductsCitiesById(id);
            if (city == null)
                throw new ArgumentException("No tier price found with the specified id");

            var productId = city.ProductID;
            var product = _productService.GetProductById(productId);
            if (product == null)
                throw new ArgumentException("No product found with the specified id");
            ...
            _productCitiesService.DeleteProductCities(city);
