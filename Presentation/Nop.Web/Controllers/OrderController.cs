...
namespace Nop.Web.Controllers
{
    public partial class OrderController : BasePublicController
    {
        ...
        protected virtual OrderDetailsModel PrepareOrderDetailsModel(Order order)
        {
            ...
            var totalamountPaidCurrency = _currencyService.ConvertCurrency(order.TotalTransactionAmount, order.CurrencyRate);
            model.TotalAmountPaid = _priceFormatter.FormatPrice(totalamountPaidCurrency, true, order.CustomerCurrencyCode, _workContext.WorkingLanguage, true);
            var amountdueCurrency = _currencyService.ConvertCurrency(order.OrderTotal - order.TotalTransactionAmount, order.CurrencyRate);
            model.DueAmount = _priceFormatter.FormatPrice(amountdueCurrency, true, order.CustomerCurrencyCode, _workContext.WorkingLanguage, true);
