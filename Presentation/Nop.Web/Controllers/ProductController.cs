...
namespace Nop.Web.Controllers
{
    public partial class ProductController : BasePublicController
    {
        ...
        private readonly IProductCitiesService _productCitiesService;
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            var orderstepsInCatalogSettings = _catalogSettings.OrderSteps;
            model.OrderSteps = orderstepsInCatalogSettings;

            var termsandconditions = _catalogSettings.TermsAndConditions + "<br/>" + product.GetLocalized(x => x.TermsAndConditions);
            ...
            var cachedPictures = _cacheManager.Get(productPicturesCacheKey, () =>
            {
                ...
                var defaultPictureModel = new PictureModel
                {
                    ...
                    DetailsImageUrl = _pictureService.GetPictureUrl(pictures.FirstOrDefault(), defaultPictureSize, !isAssociatedProduct),
                    
                };
                //Picture p =_pictureService.GetPictureById(Convert.ToInt32(Session["picid"].ToString()));
                ...
                foreach (var picture in pictures)
                {
                    pictureModels.Add(new PictureModel
                    {
                        ...
                        Title = string.Format(_localizationService.GetResource("Media.Product.ImageLinkTitleFormat.Details"), model.Name),
                        AlternateText = string.Format(_localizationService.GetResource("Media.Product.ImageAlternateTextFormat.Details"), model.Name),
                        DetailsImageUrl = _pictureService.GetPictureUrl(picture, defaultPictureSize),
                    });
            ...
            //{
            //    int i = Convert.ToInt32(Session["picid"].ToString());
            //    model.DefaultPictureModel.ImageUrl = _pictureService.GetPictureUrl(i, defaultPictureSize, !isAssociatedProduct);
            //    model.DefaultPictureModel.FullSizeImageUrl = _pictureService.GetPictureUrl(i, 0, !isAssociatedProduct);
            //    model.DefaultPictureModel.DetailsImageUrl = _pictureService.GetPictureUrl(i, defaultPictureSize);
            //    model.DefaultPictureZoomEnabled = true;
            ...
            #region settings
            model.BookedAmount = _catalogSettings.BookedAmount;
            ...
            if (_permissionService.Authorize(StandardPermissionProvider.DisplayPrices))
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        //    ProductAttributeValue price = _productAttributeService.GetProductAttributeValueById(Convert.ToInt32(Session["aid"].ToString()));
                        //    if (price!= null)
                        //    {
                        //        model.ProductPrice.PriceValue =finalPriceWithoutDiscount+ price.PriceAdjustment;
                        //        model.ProductPrice.PriceWithDiscountValue = finalPriceWithDiscount + price.PriceAdjustment;
                        //        string status = "true";
                        //        string str = "{\"ImageUrl\": \"" + model.DefaultPictureModel.ImageUrl + "\",\"Price\": " + model.ProductPrice.PriceValue + "}";
                        //        string s = "{\"Status\": \"" + status + "\",\"Data\": " + str + "}";
            ...
            model.IsHomeDecor = product.IsHomeDecor;
                ...
                if (attribute.ShouldHaveValues())
                {
                    ...
                    foreach (var attributeValue in attributeValues)
                    {
                        var valueModel = new ProductDetailsModel.ProductAttributeValueModel
                        {
                            ...
                            Discount = attributeValue.Discount
                        };
        ...
        //public string getqty(int productId = 0, int attrid = 0)
        ...
        //    if (attrid > 0)
        //    {
        //        ProductAttributeValue a = _productAttributeService.GetProductAttributeValueById(attrid);
        //        qty = Convert.ToInt32(a.Name);
...

        {
           ...
           //Session["aid"] = pictureid;
            var product = _productService.GetProductById(productId);
            if (productattrid > 0)
            {
                ProductAttributeValue a = _productAttributeService.GetProductAttributeValueById(productattrid);
                int pictureid = a.PictureId;
                decimal discount = a.Discount;
                ...
                if (pictureid != 0)
                {
                    ...
                    ProductAttributeValue price = _productAttributeService.GetProductAttributeValueById(productattrid);
                    if (price != null)
                    {
                        if (price.PriceAdjustment != 0)
                        {
                            //model.ProductPrice.CurrencyCode = _workContext.WorkingCurrency.CurrencyCode;
                            string priceofproduct = _priceFormatter.FormatPrice(finalPriceWithoutDiscount + price.PriceAdjustment);
                            string discountpriceofproduct = _priceFormatter.FormatPrice(finalPriceWithoutDiscount + price.PriceAdjustment-discount);
                            model.ProductPrice.PriceWithDiscountValue = finalPriceWithDiscount + price.PriceAdjustment;
                            decimal Final = (finalPriceWithDiscount + price.PriceAdjustment);
                            string disper = "0";
                            if (finalPriceWithoutDiscount > 0 && discount>0)
                            {
                        ...
                        else
                        {
                            ...
                            string priceofproduct = _priceFormatter.FormatPrice(finalPriceWithoutDiscount);
                            string discountpriceofproduct = _priceFormatter.FormatPrice(finalPriceWithoutDiscount - discount);
                            model.ProductPrice.PriceWithDiscountValue = finalPriceWithDiscount;
                            string disper = "0";
                            if (finalPriceWithoutDiscount > 0 && discount>0)
                            {
                ...
                {
                    return s = "{\"Status\": \"" + false + "\",\"error\": \"error\"}";
                }
            }
            ...
            else
            {
                ...
                return s = "{\"Status\": \"" + false + "\",\"error\": \"relode\"}";
            }
            ...
            //        string status = "true";
            //        string str = "{\"ImageUrl\": \"" + ImageUrl + "\",\"Price\": " + "50000" + "}";
            //        s = "{\"Status\": \"" + status + "\",\"ImageUrl\": \"" + ImageUrl + "\",\"Price\": " + "50000" + "}";
        ...
        public ActionResult ProductDetails(int productId,int updatecartitemid = 0)
        {
           // Session["aid"] = pictureid;
            ...
            //if (pictureid > 0)
            //{
            //    ProductAttributeValue a = _productAttributeService.GetProductAttributeValueById(pictureid);
        ...
        #region check zipcode 
        [HttpPost]
        public string CheckPincodeAvailibility(string pincode,int productid)
        {
            string s="";
            string Message = "";
            bool z=false;
            ...
            if (zip != null)
            {
                ...
                if (productcity != null)
                {
                    ...
                    if (deviveryid != 0)
                    {
                        var deliveryDates = _shippingService.GetDeliveryDateById(deviveryid).Name;
                        z = true;
                        Message = "Product is availaible at your location. It will be delivered in " + deliveryDates;
                    }
                    else
                    {
                        Message = "Product is available on your pin code.";
                        z = true;
                    }

                }
                else {
                    Message = "Product is not available on your pin code.";
                }
            }
